...
if(isset($_POST["zip"])){
	...
	for ($i = 0; $i < count($addressArray); $i+=2)  {
		...
		$addrDistAssoc[$addressArray[$i] . "|" . $addressArray[$i+1]] = round(($distance*0.000621371), 2);
}
		asort($addrDistAssoc);
...
<script>
      function initMap() {
		...
		$index=0;
		foreach($addrDistAssoc as $addr => $dist) {
			$latLongString = explode('|', $addr)[1];
			$latitude1 = explode(',', $latLongString)[0];
			$longitude1 = explode(',', $latLongString)[1];
    ...
    <script async defer
    src="https://maps.googleapis.com/maps/api/js?key=AIzaSyD3coReI9da-ndlYLXOePEljV0iNHgowoY&callback=initMap">
    </script>
</div>
